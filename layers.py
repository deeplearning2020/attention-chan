...
class AttentionBlock(object):
    ...
    def __call__(self, x):
        ...
        g1 = Conv2D(int(self.filters//2), kernel_size=1, padding='same')(x)

        x1 = Conv2D(int(self.filters//2), kernel_size=1, padding='same')(x)

        p1 = Conv2D(int(self.filters//2), kernel_size=1, padding='same')(x)
